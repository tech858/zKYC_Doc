...
<PackageManagerTabs packageName="install @kyc/zkyc-core" />
<PackageManagerTabs packageName="install @kyc/zkyc-core viem @x402/core @x402/evm
" />
...
<KycCodeBlock
  title="client.ts"
  code={`import { createZkycClient } from '@kyc/zkyc-core'
...
<KycCodeBlock
  title="evaluate.ts"
  code={`const decision = await zkyc.check({
...
<KycCodeBlock
  title="verify.ts"
  code={`// Merchant or API provider calls this before committing
...
<KycCodeBlock
  title="createZkycClient.ts"
  code={`createZkycClient({
...
<KycCodeBlock
  title="checkAndRefund.ts"
  code={`const result = await zkyc.checkAndRefund({
...
<KycCodeBlock
  title="checkAndRefund_return.ts"
  code={`// ALLOW + x402 configured
...
<KycCodeBlock
  title="executeRefund.ts"
  code={`const { txHash } = await zkyc.executeRefund(
...
<KycCodeBlock
  title="resolve.ts"
  code={`await zkyc.resolve(decision.checkId, true)  // approved
...
<KycCodeBlock
  title="stepup_flow.ts"
  code={`// 1. Agent receives STEP_UP
...
<KycCodeBlock
  title="verify_api.ts"
  code={`POST /api/zkyc-core/verify
...
<KycCodeBlock
  title="x402_config.ts"
  code={`const zkyc = createZkycClient({
  apiKey: '...',
...
<KycCodeBlock
  ...
  code={`import { ZkycError } from '@kyc/zkyc-core'
...
<KycCodeBlock
  title="refund_agent.ts"
  code={`import { createZkycClient, ZkycError } from '@kyc/zkyc-core'
